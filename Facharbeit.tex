...
\end{figure}
\newline
 %Hier muss die Erläuterung des fertigen Algorithmus erfolgen!!!%
...
Dann werden durch eine for-Schleife alle Leerzeichen und Kommentare entfernt. Desweiteren werden die Kommata zu Punkten gemacht,
...
Die Methode \texttt{ausgabe()} nimmt zwei Parameter an, den zweidimensionalen Array \texttt{mx} und den String \texttt{matrixName}.
Beide werden durch eine for-Schleife auf der Konsole ausgegeben.
Die Methode \texttt{multiplyAndAdd()}, welche die Parameter \texttt{lineOne, lineTwo (Integer)} und den \texttt{double factor} annimmt,
wird verwendet um, die Koeffizienten zu einer Null zu machen. Dabei wird durch eine for-Schleife über die Spalten der Matrix iteriert
...
Diese Methode wird später für die Sonderbehandlung von nicht ''korrekten'' Matrizes verwendet.
...
werden die Zeilen mithilfe einer temporären Variable getauscht, so dass am Ende die Zahl mit dem größten Koeffizienten oben steht.
% Erläutern ab Z. 143
...
da er nach einfachen Schemata funktioniert, es werden wenig Schritte benötigt.
...
Zum anderen hat der Algorithmus bei komplizierten Martizes mit
vielen Zeilen und Spalten eine große Laufzeit, die Worstcase Laufzeit würde aufgrund von doppelten for-Schleifen $ O(n x n) $ betragen.
...
Die Anwendungsmöglichkeiten, in diesen werden im Folgenden erläutert.
